...
const MaskedDiv: React.FC<MaskedDivProps> = ({
  ...
  return (
    <section
      className={`relative inline-flex flex-none overflow-hidden ${className ?? ""}`}
      style={containerStyle}
    >
      {React.cloneElement(children, {
        // ensure the child fills the mask and keeps cover behaviour
        className: `w-full h-full object-cover ${children.props.className ?? ""}`,
        // remove any width/height attributes on the child element if present (optional)
      })}
    </section>
  )
